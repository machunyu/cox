...
// Timer mode
//
static unsigned long ulTimerMode[4] = {TIMER_MODE_ONESHOT,
																			 TIMER_MODE_PERIODIC,
																			 TIMER_MODE_CAPTURE,
																			 TIMER_MODE_PWM};
...
static void xTimer001Execute(void)
{
		...
		for(i = 0; i < 4; i++)
    {
        ...
        //      
				...
				TestAssert(250 == ulTemp,
									 "xtiemr API \"TimerAPrescaleSet\" error!");  
				xTimerPrescaleSet(ulBase, TIMER_B,  240);
				ulTemp = xHWREG(ulBase + TIMER_0_GPTMTBPR);
				TestAssert(240 == ulTemp,
									 "xtiemr API \"TimerBPrescaleSet\" error!"); 
