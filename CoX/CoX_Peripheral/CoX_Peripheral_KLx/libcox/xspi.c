//*****************************************************************************
//
// spi.c - Driver for Synchronous Serial Interface.
//
...
SPI0IntHandler(void)
{
    ...
    //
    ulEventFlags = xHWREG(ulBase + SPI_S); 
    
    //
    // Clear Int flag
    //
    xHWREG(ulBase + SPI_S) |= ulEventFlags & 0x40;
                   ...
                   unsigned long ulBitRate, unsigned long ulShiftDirection)
{
    ...
    xHWREG(ulBase + SPI_BR) = (ulCnt - 1);
    xHWREG(ulBase + SPI_BR) = ((ulSCR - 1) << SPI_BR_SPPR_S);
	
    //
    // Set protocol 
    //
    ulRegVal = (ulShiftDirection | ulMode | ulProtocol);
    xHWREG(ulBase + SPI_C1) = ulRegVal;
}
...
SPIEnable(unsigned long ulBase)
{
    ...
    //
    xHWREG(ulBase + SPI_C1) |= SPI_C1_SPE;
}
...
SPIIntEnable(unsigned long ulBase, unsigned long ulIntFlags)
{
    ...
    if(ulIntFlags & 0xF0)
    {
        xHWREG(ulBase + SPI_C2) |= (ulIntFlags << 4);
    }
    else
    {
        xHWREG(ulBase + SPI_C1) |= (ulIntFlags << 4);
    }
}
...
SPIIntStatus(unsigned long ulBase, xtBoolean bMasked)
{
    ...
    // 
    return (xHWREG(ulBase + SPI_S) & SPI_S_M) ;
}
...
SPIDataPut(unsigned long ulBase, unsigned long ulData)
{
    ...
    //
    while(!(xHWREG(ulBase + SPI_S) & SPI_S_SPTEF))
    {
    }
    ...
    //
    xHWREG(ulBase + SPI_D) = (unsigned char)ulData;
}
...
SPIDataPutNonBlocking(unsigned long ulBase, unsigned long ulData)
{
    ...
    //
    if(xHWREG(ulBase + SPI_S) & SPI_S_SPTEF)
    {
        xHWREG(ulBase + SPI_D) = (unsigned char)ulData;
        return(1);
    }
...
SPIDataGet(unsigned long ulBase, unsigned long *pulData)
{
    ...
    //
    while(!(xHWREG(ulBase + SPI_S) & SPI_S_SPRF))
    {
    }
...
SPIDataGetNonBlocking(unsigned long ulBase, unsigned long *pulData)
{
    ...
    //
    if(xHWREG(ulBase + SPI_S) & SPI_S_SPRF)
    {
        *pulData = xHWREG(ulBase + SPI_D);
        return(1);
    }
...
SPIDMAEnable(unsigned long ulBase, unsigned long ulDMAFlags)
{
    ...
    //
    xHWREG(ulBase + SPI_C2) |= ulDMAFlags;
}
...
SPISingleDataReadWrite(unsigned long ulBase, unsigned long ulWData)
{
    ...
    // Write the data to the SPI.
    //
    xHWREG(ulBase + SPI_D) = (unsigned char)ulWData;
...
SPI3WireEnable(unsigned long ulBase, unsigned long ulMode)
{
    ...
    xHWREG(ulBase + SPI_C2) |= SPI_C2_SPC0;
	
    xHWREG(ulBase + SPI_C2) &= ~SPI_C2_BIDIROE;
...
SPISSSet(unsigned long ulBase, unsigned long ulSlaveSel)
{
    ...
    xHWREG(ulBase + SPI_C2) |= SPI_C2_MODFEN;
	
    xHWREG(ulBase + SPI_C1) &= ~SPI_C1_SSOE;
...
SPIClkStatusForWaitMode(unsigned long ulBase, unsigned long ulMode)
{
    ...
    xHWREG(ulBase + SPI_C2) &= ~SPI_C2_SPISWAI;
