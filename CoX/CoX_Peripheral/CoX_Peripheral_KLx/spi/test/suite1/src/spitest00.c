...
static void xspi001_SpiMasterSlaveModeSelect_test(void)
{
    ...
    for(i = 0; i < 2; i++)
    {
        for(j = 0; j < 2; j++)
        {
            ulTemp = (xSPI_MOTO_FORMAT_MODE_0 | (ulSPIOperationMode[j] << 8) | xSPI_MSB_FIRST);
            xSPIConfigSet(ulSPI[i], 200000, ulTemp);
            ulRegVal = xHWREG(ulSPI[i] + SPI_C1) & (ulSPIOperationMode[j] >> 8);
...
static void xspi001_SpiLsbMsbSet_test(void)
{
    ...
    for(j = 0; j < 2; j++)
    {
        for(i = 0; i < 13; i++)
        {
            ...
            ulRegVal = xHWREG(ulSPI[i] + SPI_C1) & ulShiftDirectionTable[i];
...
static void xspi001_SpiFormatModeSet_test(void)
{
    ...
    for(j = 0; j < 2; j++)
    {
        for(i = 0;i < 4; i++)
        {
            ...
            xSPIConfigSet(ulSPI[j], 200000, ulTemp);
            ulRegVal = xHWREG(ulSPI[j] + SPI_C1) & (ulSpiFormatMode[i] >> 16);                     
...
static void xSpi001Execute(void)
{
    ...
    {
        SPIEnable(ulSPI[i]);
        ulRegVal = xHWREG(ulSPI[i] + SPI_C1) & SPI_C1_SPE;
