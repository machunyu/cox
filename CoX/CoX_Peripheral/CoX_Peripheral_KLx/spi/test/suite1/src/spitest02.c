        ...
        for(j = 0;j < 2;j ++)
         {
            ...
            ulRegVal = xHWREG(ulSPI[i] + SPI_C2) & ulDMAFlags[j];
        ...
        for(j = 0;j < 2;j ++)
         {
            ...
            ulRegVal = xHWREG(ulSPI[i] + SPI_C1) & ulSSetFlags[j];
...
static void xspi003_SPI3Wire_test(void)
{
    ...
    for(i = 0;i < 2;i ++)
    {
	    ...
	    ulRegVal = xHWREG(ulSPI[i] + SPI_C2) & (SPI_C2_BIDIROE | SPI_C2_SPC0);
...
static void xspi003_SPIWaitMode_test(void)
{
    ...
    for(i = 0;i < 2;i ++)
    {
	    ...
	    ulRegVal = xHWREG(ulSPI[i] + SPI_C2) & ulClkMode[j];
